...
class CETEI {
    ...
    getHTML5(XML_url, callback, perElementFn){
        if (this.window.location.href.startsWith(this.base) && (XML_url.indexOf("/") >= 0)) {
    ...
    */
    fallback(names) {
      for (let name of names) {
        ...
        if (fn) {
          for (let elt of Array.from((this.dom && !this.done?this.dom:this.document).getElementsByTagName(this._tagName(name)))) {
    ...
    */
    ...
    copyAndReset(node) {
      let _clone = (n) => {
        let result = n.nodeType === Node.ELEMENT_NODE?this.document.createElement(n.nodeName):n.cloneNode(true);
        if (n.attributes) {
    ...
    hideContent(elt) {
      if (elt.childNodes.length > 0) {
        let hidden = this.document.createElement("span");
